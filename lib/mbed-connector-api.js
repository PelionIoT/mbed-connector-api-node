...
var showWarningIfUsingOldApi = true;
...
toAugment.forEach(function(k) {
  ...
  mbedConnectorApi.prototype[k] = function() {
    ...
    if (args.length >= 2 &&
      ...
      // using old API? Show warning on first usage...
      if (showWarningIfUsingOldApi) {
        console.log('\x1b[37m\x1b[43m', 'WARN' ,'\x1b[0m', 
          'mbed-connector-api - callback and options arguments have been ' +
          'switched for all functions. You are using the old syntax when ' +
          'calling \'' + k + '\'. ' + 
          'This syntax will be removed in mbed-connector-api 4.');
        showWarningIfUsingOldApi = false;
      }
