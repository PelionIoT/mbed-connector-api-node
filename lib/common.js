...
mbedConnectorApi.prototype.makeRequest = function(requestData, userCallback,
  ...
  if (this.options.accessKey) {
    ...
    if(this.options.cafile)  {
      requestObj.agentOptions = {
        ca: fs.readFileSync(options.cafile),
        checkServerIdentity: function (host, cert) {
          if(options.CN && cert.issuer.CN != options.CN )
          {
            //console.error(cert.subject.CN);
            return "Certificate not signed by requested!";
          }
          return undefined
        }
      };
